...
COPY --chown=spring:spring build/libs/GoldenTagService-0.0.1-SNAPSHOT.jar app.jar

# Copy certificates if needed
COPY --chown=spring:spring certs/ /app/certs/

# Create logs directory
RUN mkdir -p /var/log/goldentagservice && \
    chown -R spring:spring /var/log/goldentagservice
